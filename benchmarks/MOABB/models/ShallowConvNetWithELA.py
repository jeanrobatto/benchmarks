"""
Base Model: ShallowConvNet from https://doi.org/10.1002/hbm.23730.
...
NOTE - This is a modified version of the standard ShallowConvNet model.
       
       It includes an extra module, namely the ELA module from https://arxiv.org/html/2403.01123v1,
       run between the convolution layer module and the final dense layer module.

       In addition, it includes one more parameter to set the kernel size of the attention module 1D convolution.

Authors
 * Davide Borra, 2021 (Mostly)
 * Jean Robatto, 2024 (Addition of the attention_module)
"""
import torch
import speechbrain as sb
from custom_modules.LocalizationAttention import LocalizationAttention
...
class ShallowConvNetWithELA(torch.nn.Module):
    """ShallowConvNetWithELA.
    ...
    attention_kernel_size: int
        Size of kernel for the 1d convolution in the ELA module.

    Example
    -------
    >>> inp_tensor = torch.rand([1, 200, 32, 1])
    >>> model = ShallowConvNetWithELA(input_shape=inp_tensor.shape)
    ...
    def __init__(
        ...
        # The attention layer is designed to return the same shape as the conv layer.
        out = self.conv_module(
            torch.ones((1,) + tuple(input_shape[1:-1]) + (1,))
        )

        #ATTENTION Module
        self.attention_module = LocalizationAttention(channel=out.shape[1], kernel_size=attention_kernel_size)
